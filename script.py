# -*- coding: utf-8 -*-
"""
Created on Wed Apr 15 17:57:23 2020
...
def ConfirmedCasePrediction(countryCode,startingIndex, startIndexTest,label):
    ...
    print(label+" in "+ str((labelencoder_X_1.inverse_transform([countryCode]))[0]) +" min "+str (min(y_country[:,j]))+ " max " +str( max(y_country[:,j])))
    ...
    plt.title('Day vs '+str(label)+' in '+ str((labelencoder_X_1.inverse_transform([countryCode]))[0]))
    plt.xlabel("Day")
    plt.ylabel('Confirmed Cases')
    fig1.savefig(os.path.join(my_path,'Results/'+label+'/'+str((labelencoder_X_1.inverse_transform([countryCode]))[0])+'.png'))
    plt.show() 
    """
